...
const configureProviders = async () => {
    try {
        ...
        // 2. Configure SuperJara (Active, Priority 1 for airtime/data)
        await ProviderConfig.findOneAndUpdate(
            { code: 'superjara' },
            {
                active: true,
        ...
        console.log('✅ SuperJara configured: Active, Priority 1 (airtime, data)');

        // 3. Configure Topupmate (Active, Priority 2 - Fallback)
        await ProviderConfig.findOneAndUpdate(
            { code: 'topupmate' },
            {
                active: true,
