...
export const authService = {
  ...
  login: async (data: LoginData): Promise<AuthResponse> => {
    try {
      ...
      // The axios interceptor rejects with a plain object { success, message, ... }
      // so error.response is undefined here — forward the error object directly.
      throw error?.isApiError
        ? error
        : (error.response?.data || { success: false, message: error.message || 'Login failed' });
    }
  },
