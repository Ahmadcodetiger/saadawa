...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (!error.response) {
      ...
      if (
        ...
        if (url.includes('/billpayment/')) {
          errorMessage = 'The transaction is taking too long to process. Please wait 1-2 minutes and check your transaction history before trying again to prevent duplicate charges.';
        }
      }
      const errorDetails = {
    ...
    if (status === 401 && !isAuthEndpoint) {
      // Clear auth data on 401 (session expiry) — but NOT for auth endpoints
...
// Wallet Service
const walletService = {
  getWallet: async (): Promise<AxiosResponse<ApiResponse<{ wallet: Wallet }>>> => {
    try {
      const cached = await AsyncStorage.getItem('walletData');
      if (cached) {
        // Return a mock AxiosResponse structure for compatibility
        return {
          data: { success: true, data: { wallet: JSON.parse(cached) } },
          status: 200,
          statusText: 'OK',
          headers: {},
          config: {} as any
        } as AxiosResponse<ApiResponse<{ wallet: Wallet }>>;
      }
    } catch (e) {
      console.log('Error reading wallet cache', e);
    }
    // Fallback to fetch if no cache
    return walletService.fetchAndCacheWallet();
  },

  fetchAndCacheWallet: async (): Promise<AxiosResponse<ApiResponse<{ wallet: Wallet }>>> => {
    const response = await api.get<ApiResponse<{ wallet: Wallet }>>('/wallet');
    if (response.data?.data?.wallet) {
      await AsyncStorage.setItem('walletData', JSON.stringify(response.data.data.wallet));
    }
    return response;
  },
